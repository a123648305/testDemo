...
import { Popover, Button, Input,Checkbox, Select } from 'antd';
...
export default class MultiSelect extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    const { type, title, options, value } = props;
    if (type !== state.type) {
      return {
        type,
      };
  ...
  render() {
    ...
    const EMPTY_TEXT = type && type === 1 ? '请选择' : '全部';
    ...
    return (
      ...
      >
          {/* <Select notFoundContent={null} style={{width:220}} /> */}
        ...
        //   style={{ display: panelShow ? 'block' : 'none' }}
...
MultiSelect.propTypes = {
  type: PropTypes.number, //类型。1：前台。前后台多选组件功能一致，但样式有差别。
